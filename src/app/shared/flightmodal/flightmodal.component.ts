...
import {
  ...
  seatAvailable,
  bookingInfo,
} from '../methods/flightsearchObjects';
...
export class FlightmodalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!!this.flightInfo.actualFlights) {
      this.flightInfo.actualFlights.forEach((element: any) => {
        ...
        seatOptions.economy = element.seats.filter(
          (x: any) => x.seatClass == 'ECONOMY' && x.seatStatus == 'AVAILABLE'
        ).length;
        seatOptions.business = element.seats.filter(
  ...
  captureUserResponse(event: any, flight: any, index: number) {
    ...
    let economyHTMLElement = document.getElementById(
      FlightSeatType.ECONOMY + '_' + index
    ) as HTMLElement;
    let businessHTMLElement = document.getElementById(
      FlightSeatType.BUSINESS + '_' + index
    ) as HTMLElement;
