...
describe('ResetformComponent', () => {
  ...
  it("should navigate user to the 404 page if token is not valid", () => {
    ...
    const req = httpMock.expectOne(environment.accountsEndpoint + "/new-password/" + component.userToken);
    req.flush(null, {status:404, statusText:"Not Found"});

    expect(req.request.method).toBe("GET");
    expect(navigateSpy).toHaveBeenCalledWith([ '404/notfound' ], Object({ replaceUrl: true }));
  ...
  it("if passwords are not the same, then return error message", () => {
    ...
    component.onSubmitPasswordResetWithToken(testForm);
    expect(component.errorMsg).toEqual("Please enter the same password in both input boxes");
  });
  ...
  it("if passwords are the same, but do not fit criteria", () => {
    ...
    component.onSubmitPasswordResetWithToken(testForm);
    expect(component.errorMsg).toEqual("Password must be between 10 and 128 characters,"
  ...
  it("should return error response 404 if token is invalid by server", () => {
    ...
    component.onSubmitPasswordResetWithToken(testForm);
    const req = httpMock.expectOne(environment.accountsEndpoint + "/new-password");
    req.flush(null, {status:404, statusText: "Not found"});
    expect(req.request.method).toBe("POST");
    expect(component.errorMsg).toEqual("The token you provided is expired or invalid. Please resetting your password again.");
  });
  ...
  it("should navigate user to another page if process is successful", () => {
    ...
    expect(req.request.method).toBe("POST");
    expect(navigateSpy).toHaveBeenCalledWith("login/password/confirmationchange");
  });
});
