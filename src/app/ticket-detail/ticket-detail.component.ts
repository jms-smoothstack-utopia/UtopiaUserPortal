...
export class TicketDetailComponent implements OnInit {
  ...
  getTicket(): void {
    ...
    if (rawTicketId !== null) {
      ...
      this.flightRecordsService
        ...
        .subscribe((myTicket) => this.setTicket(myTicket));
    }
  }
  ...
  getFlight(): void {
    ...
    this.flightRecordsService
      ...
      .subscribe((myFlight) => this.setFlight(myFlight));
  }
  ...
  doPrettyPrinting(): void {
    ...
    (this.ticket as Ticket).timePrettyPrint = new Date(
      (this.ticket as Ticket).flightTime
    ).toDateString();
    (this.ticket as Ticket).statusPrettyPrint = (this.ticket as Ticket).status
      .replace('_', ' ')
      .toLowerCase();
    //time zones displayed are just as they get passed from the backend
    (this.flight as Flight).departurePrettyPrint = new Date(
      (this.flight as Flight).approximateDateTimeStart
    ).toTimeString();
    (this.flight as Flight).arrivalPrettyPrint = new Date(
      (this.flight as Flight).approximateDateTimeEnd
    ).toTimeString();
  }
  ...
  checkIsValidFlight(
    returnedValue: Flight | HttpErrorResponse | undefined
