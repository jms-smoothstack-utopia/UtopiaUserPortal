...
describe('SearchboxComponent', () => {
  ...
  it('click on TripRB button, then tripRBOverlay should display', () => {
    ...
    //Press the button
    let overlayButton = fixture.debugElement.nativeElement.querySelector(
      '#tripRBId'
    );
    overlayButton.click();

    //Expect toggerOverlay has been called
    expect(component.toggleOverlay).toHaveBeenCalledOnceWith(
      'tripRBOverlay',
      event
    );
  });
  ...
  it('should display popup when correct parameters are passed in to function toggleOverlay(x, y)', () => {
    ...
    const HTMLElement = fixture.debugElement.nativeElement.querySelector(
      '#tripRBOverlay'
    );
    ...
    const newHTMLElement = fixture.debugElement.nativeElement.querySelector(
      '#nonStopOverlay'
    );
  ...
  it('should change innerHTML and user choices when correct parameters are passed in to afterChoosingAnOption', () => {
    ...
    const initialMessage = fixture.debugElement.nativeElement.querySelector(
      '#tripRBId'
    ).innerHTML;
    ...
    const nextMessage = fixture.debugElement.nativeElement.querySelector(
      '#tripRBId'
    ).innerHTML;
    expect(initialMessage).not.toEqual(nextMessage);

    //Seeing if it closes the overlay
    const overlay = fixture.debugElement.nativeElement.querySelector(
      '#tripRBOverlay'
    );
  ...
  it('should throw an error if there is no user input for from airport', () => {
    ...
    component.fromAirport = '';
    component.search();
    expect(component.fromAirportErrorMsg).toEqual(
  ...
  it('should throw an error if there is no user input for to airport', () => {
    ...
    component.toAirport = '';
    component.search();
    expect(component.toAirportErrorMsg).toEqual(
      'You need to include a destination'
    );
  });
  ...
  it('should throw an error if from date is not included ', () => {
    ...
    component.fromModel = undefined;
    component.search();
    expect(component.calendarErrorMsg).toEqual(
      'You need to include a date to fly!'
    );
  });
  ...
  it('should throw an error if from date is not included ', () => {
    ...
    component.search();
    expect(component.calendarErrorMsg).toEqual(
      'Cannot include a date in the past!'
    );
  });
  ...
  it('should throw an error if to date is not included and round trip is selected ', () => {
    ...
    component.search();
    expect(component.returnCalendarErrorMsg).toEqual(
      'Need to include a return date'
    );
  });
  ...
  it('should throw an error if to date is not included and round trip is selected ', () => {
    ...
    component.search();
    expect(component.returnCalendarErrorMsg).toEqual(
      'Return date must be after trip date'
    );
  });
  ...
  it('should navigate to flight-search page if we have minimum parameters', () => {
    ...
    let testParamsInit: any = {
      ...
      departure:
        component.fromModel.year +
        '-' +
        component.fromModel.month.toLocaleString('en-US', {
          minimumIntegerDigits: 2,
          useGrouping: false,
        }) +
        '-' +
        component.fromModel.day.toLocaleString('en-US', {
          minimumIntegerDigits: 2,
          useGrouping: false,
        }),
    };
    ...
    component.search();
    expect(router.navigate).toHaveBeenCalledWith(['/flight-search'], {
      queryParams: testParamsInit,
    });
  });
  ...
  it('should navigate to flight-search page if we have minimum parameters + returntrip', () => {
    ...
    testParamsInit['return'] =
      component.toModel.year +
      '-' +
      component.toModel.month.toLocaleString('en-US', {
        minimumIntegerDigits: 2,
        useGrouping: false,
      }) +
      '-' +
      component.toModel.day.toLocaleString('en-US', {
        minimumIntegerDigits: 2,
        useGrouping: false,
      });
    testParamsInit['adults'] = component.adult;
